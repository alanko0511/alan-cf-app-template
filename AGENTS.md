...
- **Links inside a nested route are relative to its base — write them that way.** When a component renders inside `<Route path="/settings" nest>`, wouter sets the router **base** to `/settings`, and a `<Link href>` starting with `/` is resolved **relative to that base** (it's prepended). This is the one nested-routing gotcha; get it wrong and links silently double up. The convention for this template:
  - **In-section links → plain relative path, no prefix.** `<Link href="/account">` inside the `/settings` nest navigates to `/settings/account`. `<Link href="/">` → `/settings`. The same path also works with `useRoute("/account")` for active-state. **Do NOT write the full `/settings/account`** — the base is prepended again, producing `/settings/settings/account` (the doubling failure mode). The rule of thumb: a nested link's `href` should not repeat its own section prefix.
  - **Escape-to-top-level links → `~` prefix.** To leave the section for a site-root path, prefix with `~`: `<Link href="~/">` → `/` and `<Link href="~/about">` → `/about`. Without the `~`, `href="/about"` inside `/settings` would resolve to `/settings/about`. Top-level nav rendered *outside* any `nest` (e.g. the header in `App.tsx`) needs no `~` — there the base is already `/`.
  - Worked example: `routes/settings.tsx` uses both forms — relative `NavItem` links (`/`, `/account`) for the sidebar and a `~/` escape link back to the top-level app — each with an inline comment at the point of use.
